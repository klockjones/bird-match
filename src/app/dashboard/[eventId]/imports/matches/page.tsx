...
export default async function ImportMatchesPage({ params, searchParams }: ImportMatchesPageProps) {
              ...
              <article key={item.id} className="staff-card">
                ...
                {item.raw_snapshot?.status === "failed" ? (
                  <div style={{ marginTop: 12 }}>
                    <Link href={`/dashboard/${eventId}/imports/matches?errorImport=${item.id}`}>이 오류 상세 보기</Link>
                  </div>
                ) : null}
              </article>
            ))}
